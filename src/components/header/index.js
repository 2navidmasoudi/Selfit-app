...
export default ({ backButton, rightTitle }) => (
  <Header hasSubtitle style={header.headerColor} androidStatusBarColor="#313131" iosBarStyle="light-content">
    <Left style={{ flex: 1, flexDirection: 'row' }}>
      ...
      >
        <View style={{ display: backButton || 'none', justifyContent: 'center', alignItems: 'center', height: 50, width: 50 }}>
          <Icon name="arrow-back" onPress={() => Actions.pop()} style={{ color: 'white', fontWeight: 500, display: backButton || 'none', marginHorizontal: 5 }} />
        </View>
      </TouchableWithoutFeedback>
      <TouchableWithoutFeedback onPress={() => Actions.blogRoot()}>
        <View style={{ display: backButton === 'flex' ? 'none' : 'flex', justifyContent: 'center', alignItems: 'center', height: 50, width: 50 }}>
