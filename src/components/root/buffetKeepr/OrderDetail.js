...
import {Image, WebView, View, TouchableWithoutFeedback, FlatList} from 'react-native';
import {
    Container,
    Header,
    Content,
    ...
    Grid,
    Row,
    Toast, ListItem, Separator, FooterTab, Footer
} from 'native-base';
...
class OrderDetail extends Component {
    ...
    async _getBasketOrderAllBuffet() {
        try {
            let {idmember, buffetidfactor, activemethodpayed} = await this.props.order;
            let {tokenmember} = await this.props.user;
            let {tokenapi} = await this.props;
            let buffetOrder = await getBasketOrderAllBuffet(idmember, buffetidfactor, activemethodpayed, tokenmember, tokenapi, 30, 0, true);
    ...
    async _getBasketOrderMaterial() {
        try {
            ...
            let {tokenmember} = await this.props.user;
            let {tokenapi} = await this.props;
            let materialOrderid = await getBasketOrderMaterial(idmember, buffetidfactor, tokenmember, tokenapi, 30, 0, true);
    ...
    async _getMixMaterial(id) {
        try {
            ...
            let {tokenmember} = await this.props.user;
            let {tokenapi} = await this.props;
            let {Basket,PriceAll} = await getAllMixMaterial(id||0, false, tokenmember, tokenapi, 30, 0, false);
    ...
    async sendAccept(active) {
        try {
            let {tokenmember} = await this.props.user;
            let {tokenapi,order} = await this.props;
            let result = await putAcceptBuffet(order.idmember,order.idfactorbuffet,active,tokenmember,tokenapi);
    ...
    async _putAccept(active) {
        try {
            ...
            let result = await (buffetid, food.idmenufood, true, tokenmember, tokenapi);
            console.log(result);
