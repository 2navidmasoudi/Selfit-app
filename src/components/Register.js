...
class Register extends Component {
    ...
    async changeAge(age) {
        ...
        let birthdaymember = await date.format('YYYY-MM-DDTHH:mm:ss');
        await this.setState({birthdaymember});
    }

    async submitButtonPress() {
        try {
            let {fistName, lastName, mailmember, typememberid, sexmember, birthdaymember} = await this.state;
            let {phone, tokenapi, tokenmember} = await this.props.user;
            let namefamilymember = await fistName + ' ' + lastName;
            let json = await putMember(namefamilymember,
                ...
                tokenapi);
            if (json === 1) {
                let jsonInfo = await getSingleToken(tokenmember,tokenapi);
                ...
                <Content padder>
                    ...
                    <Form style={formStyle}>
                        ...
                        <Item  style={item}>
                            ...
                            <Input keyboardType='email-address' style={formInputText}
                        ...
                        <Item  style={item}>
                            ...
                            <Input keyboardType='numeric' style={formInputText}
                        ...
                        <View style={selectViewStyle}>
                            <Form style={formSelectStyle}>
                                ...
                                <ListItem style={listSelectStyle} onPress={() => this.setState({typememberid: 6})}>
                                    <Text>ورزشکار</Text>
                                    <Right>
                                        <Radio selected={this.state.typememberid === 6}
                                               onPress={() => this.setState({typememberid: 6})}/>
                                    </Right>
                                </ListItem>
                                {/*<ListItem style={listSelectStyle} onPress={() => this.setState({typememberid: 5})}>*/}
                                    {/*<Text>بوفه دار</Text>*/}
                                    {/*<Right>*/}
                                        {/*<Radio selected={this.state.typememberid === 5}*/}
                                               {/*onPress={() => this.setState({typememberid: 5})}/>*/}
                                    {/*</Right>*/}
                                {/*</ListItem>*/}
                                {/*<ListItem style={listSelectStyle} onPress={() => this.setState({typememberid: 4})}>*/}
                                    {/*<Text > باشگاه دار </Text>*/}
                                    {/*<Right >*/}
                                        {/*<Radio  selected={this.state.typememberid === 4}*/}
                            ...
                            <Form style={{flex: 2, flexDirection: 'column'}}>
                                ...
                                <Picker placeholder='انتخاب جنسیت'
