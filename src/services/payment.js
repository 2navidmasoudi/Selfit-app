...
export const getAddressPayment = async (type, token, delivery = 0, code = null, tokenapi = 'selfit.member') => {
  try {
    const response = await fetch(`${SAPI}${Payment}GetAddressRequestPayment?type=${type}&delivery=${delivery}&code=${code}&t.token=${token}&t.tokenapi=${tokenapi}`);
    const json = await response.json();
    console.log('Payment/GetAddressRequestPayment');
    console.log(json);
